...
#seq_diagram(
  ...
  diagram({
    ...
    _par("E", display-name: `ERROR 503`, color: red.lighten(50%))
  ...
  comment: [
    `PlaylistChecker` is in charge of checking if a the requested playlist _actually belongs_ to the User requesting it or trying to upload it. This is done via obtaining the User attribute from the session -- which is impossibile without extending the `HttpServlet` or `HttpFilter` classes -- and getting the needed paramaters from the request. Then, a query is performed against the database. If the result is false, then the User will face `ERROR 503: unauthorized access`.
  ],
  next_page: false,
)
...
#seq_diagram(
  ...
  comment: [
    `TrackChecker` applies a very similar pipeline `PlaylistChecker`: instead of checking the playlist, it does the same job but for the individual track.
    
    // It also obtains the User attribute from the session and the needed paramaters; if the User does not have access rights to the requested track, the response is `ERROR 503`.
  ],
  next_page: false,
)
