...
It should be noted there are two versions: a *HTML version*, which is structured as a series of separate webpages; and a *RIA version* (#ria()), which is structured as a single-page webapp. The functionalities are quite the same, the code changes mostly at a frontend level. For more information about the requirements for each version see @project-breakdown.
