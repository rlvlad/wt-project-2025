...
It should be noted there are two versions: a *HTML version*, which is structured as a series of separate webpages; and a *RIA version* (#ria())#footnote[For historic reasons, in the project is is referred as just `js`.], which is structured as a single-page webapp. The functionalities are quite the same, the code changes mostly at a frontend level. For more information about the requirements for each version see @project-breakdown.
...
_Many liberties were taken_ in regards to the DMBS and RIA:
...
- Instead of Javascript, we opted for #text(fill: rgb("#3178C6"), weight: "bold")[Typescript]
...
Then Maven will fetch all the corrected dependencies (such as the JDBC driver). We opted to use IntelliJ Idea Ultimate Edition @intellij though there are no restrictions on the IDE -- feel free to use whatever editor you want, even Eclipse, #emph[if you must]#footnote[#emph[I wrote that #emph[only] out of kindess, since I wouldn't recommend it even to my worst enemy. --- `victuarvi`.]]. Once you made sure all the dependencies are correctly installed, run the desired Tomcat configuration and let it deploy the server, which will be found at#footnote[`[version]` is either `pure_html` or `js` depending on what you run.]:
...
The credentials are stored in plain text in the database (see @register-sequence, @cut-content-optimizations), while the tracks and images are stored in `target/webapp` (see @uploadtrack-sequence).
